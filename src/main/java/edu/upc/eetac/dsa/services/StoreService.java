...
public class StoreService {
    ...
    //  DELETE ITEM
    @DELETE
    @ApiOperation(value = "Delete Item", notes = "Delete Item given Item")
    @ApiResponses(value = {
    ...
    public Response deleteItem(Item item) {
        if(item.getId()==null) return Response.status(400).build();
        if(item.getId().equals("")||item.getId().isEmpty()) return Response.status(400).build();
